...
public class OneButtonTracker extends GraphicsProgram implements MouseMotionListener {
	...
	//These are not meaningful because the program is dynamically resized to the largest screen on start.
	...
	//*****************************************************************
	//CONTROLLER STUFF
	//if not using joystick, using ps4 controller. not made private to shut up warnings.
	...
	//In general, these will be used to collect data for each trial, then their values will be copied into an Entry, then reset for the next trial.
	...
	private void initMainScreen() {
		...
		gm.otherInfo.initTrialAndScoreLabels(settings.getLabelRectBound(), counter, settings.experimentTrials, inPracticeMode, practiceText.get(0), settings.msg1, ""); //practice text looks almost useless in this case.
	...
	private void putUpNextImageSetAndWarnings() {
		...
		//GET TRIAL DATA
		Trial t = allTrials.get(counter - 1);  //gets this data even if it is tracking only, and thats fine.
		...
		//gm.recommender.changeColorPlus(t.targetLocation(true), t.color, settings.alarmIsBinary);
		//gm.recommender.changeColor(t.color, settings.alarmIsBinary);

		if (!inPracticeMode || (inPracticeMode && counter > settings.trackingPTrials)){
			if(settings.confidentMode){
				double conf = Double.valueOf(t.confidence);
				gm.recommender.changeColorBasedOnConfidence(conf, settings.alarmIsBinary);
			}
			else{
				gm.recommender.changeColor(t.color, settings.alarmIsBinary);
			}
		...
		if (settings.confidentMode) {
			gm.otherInfo.setMessage2(t.confidence);
		}
		//AUDITORY WARNING
	...
	private void initializeControllers() {
		...
		for(int i = 0; i < cs.length; i++){
			if(cs[i].getName().equals("Extreme 3D pro") && USE_JOYSTICK){
				joystick = cs[i];
	...
	private void populatePracticeTrialData() throws IOException {
		...
		for(combinedTrialsSetup = 0; combinedTrialsSetup < 8 && fin.hasNextLine(); combinedTrialsSetup++){
			...
			tempTrial.confidence = splittedLine[4];
			//Add color and clip data to trial.
			int alertPositionInSplittedString = settings.alarmIsBinary? 5 : 6; //(column numbers in file)
	...
	private void populateExperimentTrialData() {
		...
		if(settings.genIsRandom){
			generator.setupTrials_Random_Plus(allTrials);
		}
...
//					loopDiag.getBack().numMod50++;
					...
					te = new TrackerEntry(counter, p.cursor, p.movementInput, gm.trackerGraphic.trackerGraphicIsUp);
...
//		loopDiag.getBack().trialStartTime = trialStartTime;
	...
	private String getScoreDenominator(int trialsCompleted){
		...
		if(getInExperiment()){
			possiblePoints = trialsCompleted * 15;
			...
			if(trialsCompleted <= settings.trackingPTrials){
				...
				possiblePoints = (settings.trackingPTrials * 10) + ((trialsCompleted - settings.trackingPTrials) * 15);
