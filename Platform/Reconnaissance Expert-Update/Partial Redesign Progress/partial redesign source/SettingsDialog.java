...
//You can put text directly with JRadioButtons... although, this may not be great for lining stuff up.
...
public class SettingsDialog extends JDialog implements ActionListener {
	...
	private class ReliabilityProfilePanel extends JPanel {
		...
		private JLabel profileLabel = new JLabel("Reliability Information To Display During Experiment Trials:");
		private ButtonGroup profileButtonGroup = new ButtonGroup();
		
		//overall
		private JRadioButton overallButton = new JRadioButton();
		private JLabel overallLabel = new JLabel("Overall (Uses line 1 Message)");
		
		//rate. Based on true state
		private JRadioButton rateButton = new JRadioButton();
		private JLabel rateLabel = new JLabel("Hit / Correct Rejection Rate (Uses line 1 and 2 Message)");
		
		//value. Based on what detector says
		private JRadioButton valueButton = new JRadioButton();
		private JLabel valueLabel = new JLabel("Positive / Negative Predictive Value (Uses line 1 and 2 Message)");
		
		//none
		private JRadioButton noneButton = new JRadioButton();
		private JLabel noneLabel = new JLabel("None (Uses nothing)");
		...
		public void updateReliabilityMessages(){
			...
			if(overallButton.isSelected()){
				...
				messagePanel.setMessage1("Reliability: " + (int)(calculatedReliabilityPanel.getReliability(ReliabilityCalculator.ReliabilityType.OVERALL) * 100) + "%");
				messagePanel.setMessage2("");
			}
				...
				messagePanel.setMessage1("Reliability when there is truly a threat: " + threatReliability + "%");
				messagePanel.setMessage2("Reliability when there is truly not a threat: " + noThreatReliability + "%");
			}
				...
				messagePanel.setMessage1("Reliability of the clear signal: " + clearReliability + "%");
				messagePanel.setMessage2("Reliability of the danger signal: " + dangerReliability + "%");
			}
	...
	//HERE ARE LEGACY PRE-SETTINGS
 	private void setToLegacy(int reliability, boolean isBinary){
		...
		trialPanel.setTrackPrac(30);
		trialPanel.setCombPrac(8);
		trialPanel.setTrials(100);
		...
		if(isBinary){
			...
			switch(reliability){
				case 70 : {
					binaryQuotaPanel.setBinaryQuotaList(new ArrayList<Integer>(Arrays.asList(9, 21, 11, 59)));
				}break;
