...
public class GraphicsManager {
	...
	public void changeToImageScreen(){
	...
	public class OtherInfo{
		...
		public void initTrialAndScoreLabels(ElementRectBound bound, int counter_in, int expTrials_in, boolean inPracticeMode_in, String practiceText0_in, String message1_in, String message2_in){
			...
			//Message stuff
			//MESSAGE 1 LABEL
			msg1 = new GLabel(message1_in);
			msg1.setFont(myFont);
			msg1.setLocation(trialNumber.getX() + (2 * trialNumber.getBounds().getWidth()), trialNumber.getY());
			programCanvas.add(msg1);
			
			//MESSAGE 2 LABEL
			msg2 = new GLabel(message2_in);
			msg2.setFont(myFont);
			msg2.setLocation(msg1.getX(), msg1.getY() + msg1.getBounds().getHeight());
	...
	public class Recommender{
		...
		public void init(ElementRectBound bound){
			...
			automationRecommendation = new GRect[settings.alarmIsBinary ? 2 : 4];
			
			double startX = bound.getTopLeft().getX();
			double startY = bound.getTopLeft().getY();
			double width = bound.getWidth() / automationRecommendation.length; 
			...
			for (int i = 0; i < automationRecommendation.length; i++) {
				if ((isBinaryAlarm_in && ExpSettings.BINARY_COLORS[i].equals(c))
					...
					automationRecommendation[i].setFillColor(
							modifyAlpha(automationRecommendation[i].getColor(), TrackerConstants.ACTIVE_ALARM_ALPHA)
					);
				} else {
					
					automationRecommendation[i].setFillColor(
							modifyAlpha(automationRecommendation[i].getColor(), TrackerConstants.INACTIVE_ALARM_ALPHA)
					);
				}
			}
